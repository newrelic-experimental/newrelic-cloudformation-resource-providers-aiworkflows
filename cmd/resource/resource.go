...
func wrap(f func(client *client.GraphqlClient, model model.Model) (event handler.ProgressEvent, err error), req handler.Request, currentModel *Model, prevModel *Model) (event handler.ProgressEvent, err error) {
   ...
   // c := client.NewGraphqlClient(req, &typeName, sm, eh, NewResultHandler())
...
func Update(req handler.Request, prevModel *Model, currentModel *Model) (handler.ProgressEvent, error) {
   log.Debugf("Update: model in: %+v", *currentModel)
   evt, err := wrap(update, req, currentModel, prevModel)
   log.Debugf("Update: model out: %+v", evt.ResourceModel)
   return evt, err
}
